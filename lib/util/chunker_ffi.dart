...
typedef ChunkerNextNative = ffi.Int32 Function(ffi.Int32, ffi.Pointer<ffi.Uint8>, ffi.Int32);
...
class ChunkerFFI {
  ...
  static ffi.DynamicLibrary _loadLibrary() {
    ...
    try {
      ...
      if (Platform.isMacOS) {
        final libPath = '$projectRoot/chunker-ffi/libchunker.dylib';
        _dylib = ffi.DynamicLibrary.open(libPath);
      } else if (Platform.isLinux) {
        final libPath = '$projectRoot/chunker-ffi/libchunker_linux_${_getArch()}.so';
        _dylib = ffi.DynamicLibrary.open(libPath);
      } else if (Platform.isWindows) {
        final libPath = '$projectRoot/chunker-ffi/libchunker.dll';
        _dylib = ffi.DynamicLibrary.open(libPath);
      ...
      throw Exception('Failed to load chunker library: $e. Make sure to build it first using chunker-ffi/build.sh');
    }
  ...
  static String _getArch() {
    if (ffi.Abi.current() == ffi.Abi.macosArm64 || ffi.Abi.current() == ffi.Abi.linuxArm64) {
      return 'arm64';
    }
    return 'amd64';
  }
  ...
  ChunkerFFI() {
    ...
    _chunkerNew = dylib.lookupFunction<ChunkerNewNative, ChunkerNewDart>('ChunkerNew');
    _chunkerNext = dylib.lookupFunction<ChunkerNextNative, ChunkerNextDart>('ChunkerNext');
    _chunkerClose = dylib.lookupFunction<ChunkerCloseNative, ChunkerCloseDart>('ChunkerClose');
    _chunkerGetMinSize = dylib.lookupFunction<ChunkerGetSizeNative, ChunkerGetSizeDart>('ChunkerGetMinSize');
    _chunkerGetMaxSize = dylib.lookupFunction<ChunkerGetSizeNative, ChunkerGetSizeDart>('ChunkerGetMaxSize');
  }
